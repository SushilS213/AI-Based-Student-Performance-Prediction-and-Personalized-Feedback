...
def dashboard1():
    ...
    sns.lineplot(x=x,y=y,c = "m",marker="s")
    plt.xlabel("Subject")
    plt.ylabel("Marks")
    plt.title("Over All Subject")
    plt.tight_layout()
...
def predict():
    ...
    # Numerical Inputs
    Attandance = float(request.form["Attendance"])
    Maths = float(request.form["Maths"])
    DS = float(request.form["DS"])
    OS = float(request.form["OS"])
    DBMS = float(request.form["DBMS"])
    AI = float(request.form["AI"])
    Study_houre = float(request.form["Study_houre"])
    Age = float(request.form["Age"])

    # Auto Calculate Final Percentage
    Final_Percentage = (Maths + DS + OS + DBMS + AI) / 5
    ...
    Branch_AIDS = 1 if Branch == "AIDS" else 0
    Branch_AIML = 1 if Branch == "AIML" else 0
    Branch_CSE = 1 if Branch == "CSE" else 0
    Branch_CSIT = 1 if Branch == "CSIT" else 0
    Branch_DS = 1 if Branch == "DS" else 0
